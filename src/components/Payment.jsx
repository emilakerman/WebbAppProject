...
const Payment = () => {
  ...
  const addMoviesToBought = async () => {
    ...
    if (user) {
      ...
      const expiryTime = new Date(currentTime.getTime() + 24 * 60 * 60 * 1000); // 1 day expiry
      ...
      try {
        await Promise.all(moviesToAdd.map(async (movie) => {
          await addDoc(rentedMoviesRef, movie);
        }));
